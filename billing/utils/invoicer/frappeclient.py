...
class FrappeClient(object):
	...
	def get_list(self, doctype, fields=["*"], filters=None, limit_start=0, limit_page_length=10000, order_by=None):
	...
	def post_process(self, response):
		if not response.ok:
			return { 'ok': response.ok, 'status_code': response.status_code,
					 'reason': response.reason, 'response' : response.text }
		try:
			rjson = response.json()
		except ValueError:
			return { 'ok': False, 'status_code': 415, 'reason': "Json conversion of response failed", 'response': response.text}


		if rjson and ('exc' in rjson) and rjson['exc']:
			return { 'ok': False, 'status_code': 415, 'reason': "Json conversion of response failed with: " + str(rjson['exc']), 'response': response.text}
		rv = None
		if 'message' in rjson:
			rv = rjson['message']
		elif 'data' in rjson:
			rv = rjson['data']
		return { 'ok': True, 'status_code': response.status_code, 'reason': "OK" , 'response': rv}
...
# Direct query of Frappe DB
def dbQuery (qry):
	dbconfig = {"host": "frappedb.c59hcehledvt.ap-south-1.rds.amazonaws.com",
				"user": "frappedb", "password": "Frappe#2025", "database": "frappedb",
